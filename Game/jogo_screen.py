...
class JogoScreen:
    ...
    def create_board(self):
        ...
        new_w_button = self.button_select.get_width() // 3
        new_h_button = self.button_select.get_height() // 3
        self.button_select = pygame.transform.smoothscale(self.button_select, (new_w_button, new_h_button))
        self.rect_button_select = pygame.Rect(375, 500, new_w_button, new_h_button)

        self.atk_select_text = pygame.image.load('Game/Sprites/AtkText.png').convert_alpha()
        self.atk_select_text = pygame.transform.smoothscale(self.atk_select_text, (consts_and_variables.TEXT_ATK_DEF_SIZE[0], consts_and_variables.TEXT_ATK_DEF_SIZE[1]))
        self.rect_select_atk_text = self.atk_select_text.get_rect(center=(self.screen_width / 2, self.screen_height / 2))

        self.def_select_text = pygame.image.load('Game/Sprites/DefText.png').convert_alpha()
        self.def_select_text = pygame.transform.smoothscale(self.def_select_text, (consts_and_variables.TEXT_ATK_DEF_SIZE[0], consts_and_variables.TEXT_ATK_DEF_SIZE[1]))
        self.rect_select_def_text = self.def_select_text.get_rect(center=(self.screen_width / 2, self.screen_height / 2))
    ...
    def create_hands(self):
        ...
        for i, (rank, code) in enumerate(player_hand):
            ...
            self.player_cards.append(Card(f'Game/Sprites/Cards/{code}.png', pos, size, rank, code, self.player))
        ...
        for i, (rank, code) in enumerate(enemy_hand):
            ...
            self.enemy_cards.append(Card(f'Game/Sprites/Cards/{code}.png', pos, size, rank, code, self.enemy))
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if self.rect_button_select.collidepoint(pos):
                        ...
                        if self.actual_state == GameStates.PLAYER_SELECTING_ATK_CARDS:
                            if self.total_atk_cards_select == 5:
    ...
    def draw(self, screen):
        ...
        if self.actual_state == GameStates.PLAYER_SELECTING_ATK_CARDS:
            ...
            screen.blit(self.atk_select_text, self.rect_select_atk_text)

        if self.actual_state == GameStates.PLAYER_SELECTING_DEF_CARDS:
            screen.blit(self.button_select, self.rect_button_select)
            screen.blit(self.def_select_text, self.rect_select_def_text)
    ...
    def next_round(self):
        ...
        self.deck_controller.discard_cards([(c.rank, c.code) for c in self.player_cards])

        self.deck_controller.discard_cards([(c.rank, c.code) for c in self.enemy_cards])
